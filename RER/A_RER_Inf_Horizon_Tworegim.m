...
load('data5.mat')
for j=0:1;
    Ind_var={ 'rer' ['drt' num2str(j)]  ['EE' num2str(j)]  'Con' ['dr' num2str(j)] ['dp' num2str(j)]  'dum1' 'dum2' 'dum3' 'dum4' 'NFA_MB' 'Rs' 'Rh'  }; %'NFA_MB'  'tot' 'Rs' 'Rh'
    % find dummies position
    Ind_var={ 'rer' ['drt' num2str(j)]   ['dr' num2str(j)]  'Con' ['dp' num2str(j)]  'dum1' 'dum2' 'dum3' 'dum4'  }; %'NFA_MB'  'tot' 'Rs' 'Rh'['EE' num2str(j)]
    ...
    for i=0:length(Regims)
        ...
        ahat=RER_Jackknife_inf_hor(BX0,X3,Y-X2*BX0,Ind_var2,Ind_var2(dum_var2));
        [bsBX,bsB2,CI]=RER_BootStrap_inf_hor(BX.',zeros(1,size(Y,2)),Y-X2*((X2.'*X2)\(X2.'*Y)),size(Y,1),endo_var,ahat);%mean(Y)
        
        Res.(['H' num2str(j) '_R' num2str(i)])=struct('CI',CI,'BX',bsBX,'B2',bsB2);
        if j>1
            Res.(['H' num2str(j) '_R' num2str(i)]).CI.BCa.B2=[];
            Res.(['H' num2str(j) '_R' num2str(i)]).CI.Prc.B2=[];
        end
