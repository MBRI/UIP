function [beta2,error,PROBLEM,MM,VV]=kcohnx(Y,X,a1,hlast,beta0,P00,L,MAXTRYS)
Q=0;
PROBLEM=0;
...
i=T;  %period t
p00=squeeze(ptt(i,:,:)); 
beta2(1,:)=beta_tt(i:i,:)+(wa(1:1,:)*chol(p00));   %draw for beta in period t from N(beta_tt,ptt)
error=Y-X*reshape(beta2(1,:),N*L+1,N);  %var residuals
roots(1)=stability(beta2(1,:)',N,L);
